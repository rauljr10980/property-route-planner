...
import { MapPin, Upload, X, Navigation, AlertCircle, CheckCircle } from 'lucide-react';
  ...
  const processWithMapping = async () => {
    ...
    try {
      ...
      if (geocodedData.length === 0) {
        throw new Error(`Could not geocode any addresses. Failed on all ${failedAddresses.length} addresses.`);
      }
      ...
      if (failedAddresses.length > 0) {
        alert(`Successfully geocoded ${geocodedData.length} properties. ${failedAddresses.length} addresses failed.`);
      }
